...
* *Cohort or Cross-Sectional* input parameters:two-sided confidence level, power, ratio of unexposed to exposed, percent outcome in unexposed group, and one of the three remaining input. These three inputs are: risk ratio, odd ratio, and percent outcome in exposed group. 

* *Unmatched Case-Control* input parameters: 

Positive aspects of *StatCalc* includes:
* The output lists sample sizes for most popular confidence levels. Seven options in the *Population Survey* module, six options in the *Cohort or Cross-Sectional* module, 
...
Formulas for sample sizes with the correction factor as described in Fleiss (1981) cf. Sullivan and Soe (2007):

$$
n_{1cc} = \frac{n_1}{4} \left[ 1 + \sqrt{1 + \frac{2(r+1)}{n_1r|p_2 - p_1|}} \right]
$$
...
compute_n_kelsey_or <- function(confidence_level = 0.95, power = 0.8, OR, r = 1, p2) {
  ...
  n1 <- ceiling(n1)
  n2 <- r * n1
  #print(paste0("z_alpha_2_tails : ", as.character(z_alpha_2tails)))
  #print(paste0("z_power : ", as.character(z_power)))
...
Given identical input parameters as in set 4, the *Cohort or Cross-Sectional* module also generated $p_1$ = 14%, $n_1$ = 361, and $n_2$ = 361.
