...
* The labeling of input parameter "Clusters" and "Cluster Size" can be confusing for users. For example, the *Population Survey*'s window of  *StatCalc* provides an instruction: "For simple random sampling, leave design effect and clusters equal to 1". To obtain the resulting sample size, users must look at the values under the output parameter "Cluster Size". That is, the output of "Cluster Size" is the sample size calculated by the *Population Survey* module. 

* The labeling of output parameter "Cluster Size" may be questionable. Two cases are provided below.

a. If users are required to enter a value of 1 for input parameter "Clusters", then the "Cluster Size" should equal to the value of input parameter "Population size". But, the value of "Cluster Size" is the resulting sample size instead. 

b. Another case pertains to the values of "Cluster Size" when input parameter "Clusters" is larger than 1. The *Population Survey* guideline defines input parameter "Clusters" as follow: "This input designates the number of clusters in the population survey." Following this definition and assuming that clusters are of equal sizes, the value of "Cluster Size" should equal to the value of "Population size" divided by the value of "Clusters". However, the value of "Cluster Size" does not equal to the value of "Population size" divided by the value of "Clusters". If elements are clustered, "Cluster Size" seems to refer to the number of elements to be sampled from each cluster (i.e., the number of secondary sampling units to be sampled).
