...
```{r}
type_1err_hyper <- phyper(q=33, m = N*p_0, n = (N-N*p_0), k = n)
type_1err_hyper
```
```{r}
d0 <- dhyper(x=0, m = N*p_0, n = (N-N*p_0), k = n)
d0
```
```{r}
d1 <- dhyper(x=1, m = N*p_0, n = (N-N*p_0), k = n)
d1
```
...
```{r}
phyper(q=1, m = N*p_0, n = (N-N*p_0), k = n)
```
