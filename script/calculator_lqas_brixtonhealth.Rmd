...
The *brixtonhealth* calculator calculates decision rule (decision threshold), actual type 1 error, and actual type 2 error. To obtain these three outputs, a user provides as inputs: the population size, the upper proportion threshold to construct the null hypothesis, the lower proportion threshold, sample size, maximum tolerable type 1 error, and maximum tolerable type 2 error. The actual type 1 error and actual type 2 error are calculated for the decision rule that *brexihealth* generates and the sample size, population size, upper threshold, and lower threshold that users specified.*brixtonhealth* calculation is based on hypergeometric distribution.
...
The calculator outputs are: decision rule, actual $\alpha$ error, and actual $\beta$ error. The calculation of actual $\alpha$ error and actual $\beta$ error that *brixtonhealth* calculator generates are correct for the decision rule that the calculator produces. The decision rule that *brixtonhealth* generates fulfills the conditions of lower than the maximum tolerable type 1 error and the maximum tolerable type 2 error that user specified. Section 3 provides details of how we checked the calculator calculation.
...
*check 11:* input: upper threshold = 0.7 and other inputs are their default values, outputs: decision rule = 30, actual $\alpha$ error = 0.0757, and actual $\beta$ error = 0.0517. Shown below, the calculator generates correct outputs.
...
*check 12:* input: upper threshold = 0.5 and other inputs are their default values, outputs: warning "Upper threshold must be above lower threshold".

*check 13:* input: upper threshold = none (left missing) and other inputs are their default values, outputs: decision rule = 0, actual $\alpha$ error = NaN, and actual $\beta$ error = 0. Calculator provides some outputs even when upper threshold is missing.

*check 14:* input: lower threshold = 0.4 and other inputs are their default values, outputs: decision rule = 30, actual $\alpha$ error = 0.0005, and actual $\beta$ error = 0.0009. Shown below, the calculator generates correct outputs.
...
*check 15:* input: lower threshold = 0 and other inputs are their default values, outputs: decision rule = 0, actual $\alpha$ error = 0.000, and actual $\beta$ error = 0.000. Shown below, given decision rule = 0, the calculator generates correct outputs.
...
*check 16:* input: population size = 1 and other inputs are their default values, outputs: warning "sample size must be smaller than population size"

*check 17:* input: population size = 1,000,000 and other inputs are their default values, outputs: decision rule = 33, actual $\alpha$ error = 0.0110, and actual $\beta$ error = 0.0057. Shown below, given decision rule = 33, the calculator generates correct outputs.
...
* It is not completely clear how the calculator utilizes the maximum tolerable $\alpha$ error and the maximum tolerable $\beta$ error that users specify. The calculator' outputs may remain the same even when the value of maximum tolerable $\alpha$ error or the maximum tolerable $\beta$ error varies. For example, see check #4, #5, #6 in *Input validation check* section.

* The power achieved from the sampling is not directly. To obtain the power achieved from the sampling plan, users must conduct an additional calculation, that is, 1 - actual $\beta$ error. 
