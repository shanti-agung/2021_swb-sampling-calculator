...
*check 17:* input: population size = 1,000,000 and other inputs are their default values, outputs: decision rule = 33, actual $\alpha$ error = 0.0144, and actual $\beta$ error = 0.0077. Shown below, given decision rule = 33, the calculator generates correct outputs.
...
* It is not always clear how the calculator utilizes the maximum tolerable $\alpha$ error and the maximum tolerable $\beta$ error that users specify. The calculator' outputs may remain the same even when the value of maximum tolerable $\alpha$ error or the maximum tolerable $\beta$ error varies. For example, see check #4, #5, #6 in the *Section 3: Input validation check* section.

* The power achieved from the sampling is not directly available for users. To obtain the power achieved from the sampling plan, users must conduct an additional step of calculation, that is, 1 - actual $\beta$ error. 
